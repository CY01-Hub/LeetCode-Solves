<div align="center">

# 🧩 LeetCode Solutions Repository

<img src="https://readme-typing-svg.herokuapp.com?font=JetBrains+Mono&size=26&duration=3000&pause=800&color=38BDF8&center=true&vCenter=true&width=700&lines=Solving+Problems+Every+Day+%F0%9F%94%A5;Data+Structures+%26+Algorithms;Improving+Problem+Solving+Skills;One+Problem+At+a+Time+%F0%9F%9A%80" />

<br>

<p>
A collection of my LeetCode solutions focused on improving 
<strong>Data Structures, Algorithms, Logical Thinking, and Coding Efficiency.</strong>
</p>

</div>


---

# 📌 About This Repository

This repository contains my solutions to LeetCode problems that I solve while improving my problem-solving skills and strengthening my understanding of Data Structures and Algorithms.

The main goal of this repository is not only to store solutions but also to document my learning journey — from understanding concepts to implementing optimized solutions.

Each solution focuses on:

- ✅ Clean and readable code
- ✅ Efficient approaches
- ✅ Understanding algorithmic patterns
- ✅ Improving time and space complexity analysis


---

# 🧠 Topics Covered


<div align="center">

| Category | Topics |
|---|---|
| 📦 Arrays | Searching, Sorting, Two Pointer, Sliding Window |
| 🔗 Linked List | Traversal, Reversal, Fast & Slow Pointer |
| 🥞 Stack | Monotonic Stack, Expression Problems |
| 🚶 Queue | BFS, Deque, Priority Queue |
| 🌳 Trees | Binary Trees, BST, DFS, BFS |
| 🕸️ Graphs | Traversal, Shortest Path, Graph Algorithms |
| 🔁 Recursion | Backtracking, Divide & Conquer |
| ⚡ Dynamic Programming | Memoization, Tabulation |
| 🔍 Searching | Binary Search, Optimization Problems |

</div>


---

# 🛠️ Languages Used


<div align="center">

<img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white"/>

<img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white"/>

<img src="https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white"/>

</div>




---

# 🎯 Goals


- [x] Build strong DSA fundamentals
- [x] Practice coding consistently
- [ ] Master advanced algorithms
- [ ] Improve competitive programming skills
- [ ] Solve 500+ LeetCode problems


---

# 📚 Currently Improving


- Data Structures & Algorithms
- Algorithm Optimization
- Problem-Solving Patterns
- Competitive Programming Techniques


---

# 🔗 Connect


<div align="center">


<a href="https://leetcode.com/Dhrubo-Dey">

<img src="https://img.shields.io/badge/LeetCode-Dhrubo--Dey-FFA116?style=for-the-badge&logo=leetcode&logoColor=black"/>

</a>


<a href="https://github.com/CY01-Hub">

<img src="https://img.shields.io/badge/GitHub-CY01--Hub-181717?style=for-the-badge&logo=github"/>

</a>


</div>


---

<div align="center">

⭐ Consistency beats perfection.

<br>

🚀 One problem solved today makes tomorrow's problem easier.

</div>

